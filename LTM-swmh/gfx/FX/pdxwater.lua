...
float4 main( VS_OUTPUT_WATER Input ) : COLOR
{
	...
	float fresnel = saturate( dot( -vEyeDir, normal ) ) * 0.25f;
	...
	float3 outColor = refractiveColor * ( 0.75f - fresnel ) + reflectiveColor * fresnel;
...
float4 main( VS_OUTPUT_WATER Input ) : COLOR
{
	...
	float3 outColor = refractiveColor * ( 0.65f - fresnel ) + reflectiveColor * fresnel;
